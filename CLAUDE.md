...
| `exTryBuild(questions, aspects, oiList, favoriOi, favoriTarget, fixedTargets, maxPoints, rng)` | Construction en deux phases : (1) réserve d'abord les OI à cible exacte (fixes + favorite, la plus contrainte en premier, candidats au coût marginal le plus bas) ; (2) glouton MRV habituel pour le reste (aspects aux candidats les plus rares traités en premier). Retourne `null` si un aspect n'a plus de candidat valide (déjà utilisé, conflit d'aspect, candidat couvrant plus d'un aspect à la fois, clé de diversité déjà prise pour cette OI, plafond d'OI atteint, budget de points dépassé), ou si l'OI favorite/les OI à cible fixe ne totalisent pas exactement leur cible en fin de construction |
...
| `exGenererExamen({questions, periode, aspects, oiList, favoriOi, maxPoints, rng})` | Boucle imbriquée `exTryBuild` sur les paliers de cible décroissants pour l'OI favorite (`exFavoriTargetLevels`, boucle extérieure) puis pour les cibles fixes (`exFixedTargetLevels`, boucle intérieure) — si le nombre exact visé est infaisable dans le budget de points, redescend d'un cran plutôt que d'échouer. Trie le résultat selon l'ordre canonique des aspects du programme |
