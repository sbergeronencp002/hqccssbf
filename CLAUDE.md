...
| `examen.html` | **Générateur d'examens** — pour une période choisie, sélectionne automatiquement une question dédiée par aspect du programme (jamais deux fois, jamais deux aspects partagés par une même question — le nombre de questions égale toujours le nombre d'aspects, 14 pour P5), budget de points ≤ 25, variété des 8 OI garantie, jamais deux fois la même clé de diversité (sous-tag, ou nombre de documents pour « Mettre en relation des faits ») pour une même OI, plafond dur de 2 pour « Mettre en relation des faits » (`EX_OI_HARD_CAP`), + l'OI « favorite » choisie par l'enseignant (cause-conséquence / similitudes-différences / liens de causalité) apparaît exactement 2 fois (dégrade à 1 si infaisable dans le budget), en préférant les questions à documents riches. Permet un remplacement manuel question par question (🔀, même aspect(s), budget/diversité/plafond respectés). Génère 3 DOCX distincts : questionnaire (énoncés + réglettes + espaces réponse, documents renumérotés en chiffres), dossier documentaire (tous les documents, numérotés globalement, 2/rangée), guide de correction. Lecture seule (aucune écriture sur `questions.js`), JS autonome inline (pas de dépendance à `app.js`) |
...
| `exComputeOiQuota(oiList, favoriOi, favoriTarget)` | Quota par OI : 1 de base chacune (variété), et exactement `favoriTarget` pour l'OI favorite si celle-ci dépasse 1 |
| `exOiCap(oi, favoriOi, favoriTarget)` | Plafond effectif pour une OI : le plus contraignant entre `EX_OI_HARD_CAP` (ex. 2 pour « Mettre en relation des faits », indépendant du favori) et `favoriTarget` si `oi` est l'OI favorite |
| `exTryBuild(questions, aspects, oiList, favoriOi, favoriTarget, maxPoints, rng)` | Une tentative de construction (glouton + MRV : aspects aux candidats les plus rares traités en premier) ; retourne `null` si un aspect n'a plus de candidat valide (déjà utilisé, conflit d'aspect, candidat couvrant plus d'un aspect à la fois, clé de diversité déjà prise pour cette OI, plafond d'OI atteint, budget de points dépassé), ou si l'OI favorite ne totalise pas exactement `favoriTarget` occurrences en fin de construction |
| `exGenererExamen({questions, periode, aspects, oiList, favoriOi, maxPoints, rng})` | Boucle `exTryBuild` sur des paliers de cible décroissants pour l'OI favorite (`EX_FAVORI_TARGET_LEVELS` = [2, 1, 0]) — si le nombre exact visé est infaisable dans le budget de points (ex. une OI dont tous les candidats coûtent 3 points), redescend d'un cran plutôt que d'échouer. Trie le résultat selon l'ordre canonique des aspects du programme |
...
⚠️ La cible de 2 pour l'OI favorite (`EX_FAVORI_TARGET_LEVELS`) n'est pas une garantie absolue : si tous ses candidats coûtent par exemple 3 points chacun et que le budget ne permet pas d'en placer 2 dans les aspects où elle a des candidats disponibles, l'algorithme redescend à 1 plutôt que d'échouer — `examen.html` affiche le nombre réel obtenu.
