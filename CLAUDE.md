...
| `EX_FAVORI_SCENARIOS_BY_PERIODE` | Scénarios propres à une période ET à l'OI favorite, essayés en premier par `exGenererExamen` (voir ci-dessus) — chaque niveau ajuste `targets` (cibles fixes) et/ou `extraSlots` (occurrences en plus d'un aspect déjà répétable). P1 : Causes et Différences n'ont chacune que 2 sous-tags distincts dans les données de cette période, donc un 3ᵉ niveau y est mathématiquement impossible (le scénario échoue proprement et retombe sur la base) tant que le contenu n'est pas enrichi via admin.html — Causalité, elle, n'a pas cette limite (pas de sous-tag) et atteint bien sa cible de 3 (+1 slot sur « Rapports sociaux chez les Autochtones », 25 points exactement) |
