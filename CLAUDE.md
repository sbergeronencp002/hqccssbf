...
| `exOiCap(oi, favoriOi, favoriTarget, fixedTargets)` | Plafond effectif pour une OI dans cette tentative : le plus contraignant entre `exEffectiveHardCap`, sa cible fixe (`fixedTargets`) et `favoriTarget` si `oi` est l'OI favorite |
| `exTryBuild(questions, slots, oiList, favoriOi, favoriTarget, fixedTargets, maxPoints, rng)` | Construction en deux phases sur les `slots` (voir `exBuildAspectSlots` — un slot ordinaire est un aspect seul, un slot fusionné regroupe plusieurs aspects couverts par une seule question) : (1) réserve d'abord les OI à cible exacte (fixes + favorite, la plus contrainte en premier, candidats au coût marginal le plus bas) ; (2) glouton MRV habituel pour le reste (slots aux candidats les plus rares traités en premier). Retourne `null` si un slot n'a plus de candidat valide (déjà utilisé, clé de diversité déjà prise pour cette OI, plafond d'OI atteint, budget de points dépassé), ou si l'OI favorite/les OI à cible fixe ne totalisent pas exactement leur cible en fin de construction |
| `exFixedTargetLevels()` | Paliers de dégradation des cibles fixes (`EX_OI_FIXED_TARGET`), du plus ambitieux au plancher de 1 chacune |
| `exGenererExamen({questions, periode, aspects, oiList, favoriOi, maxPoints, rng})` | Construit les `slots` (`exBuildAspectSlots`) et l'OI réellement atteignables dans cette période (`effectiveOiList` — exclut une OI totalement absente, ex. « Déterminer des changements et des continuités » en P1, la toute première période du programme : ni la variété des 8 OI ni une cible fixe ne peuvent alors l'exiger), puis boucle imbriquée `exTryBuild` sur les paliers de cible décroissants pour l'OI favorite (`exFavoriTargetLevels`, boucle extérieure) puis pour les cibles fixes (`exFixedTargetLevels`, boucle intérieure) — si le nombre exact visé est infaisable dans le budget de points, redescend d'un cran plutôt que d'échouer. Trie le résultat selon l'ordre canonique des aspects du programme |
