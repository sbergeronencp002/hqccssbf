...
| `exReorderNoAdjacentOi(list)` | Réordonne la liste triée par aspects pour qu'aucune OI ne se retrouve deux fois de suite — algorithme « reorganize string » (groupe par OI, place les groupes les plus fréquents en premier aux indices pairs puis impairs), garanti tant qu'aucune OI ne dépasse ceil(n/2) occurrences |
