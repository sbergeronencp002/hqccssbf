...
### Cache-bust actuel
`app.js?v=52`, `style.css?v=32`, `oi-config.js?v=1`, `questions-io.js?v=3`, `filters.js?v=1`, `reglettes.js?v=3` (admin) — incrémenter à chaque changement majeur.
...
| `exTryBuild(questions, slots, oiList, favoriOi, favoriTarget, fixedTargets, maxPoints, rng, relaxDiversityOis)` | Construction en phases sur les `slots` (voir `exBuildAspectSlots` — un slot ordinaire est un aspect seul, un slot fusionné regroupe plusieurs aspects couverts par une seule question, un aspect répété en génère plusieurs) : (1) réserve d'abord les OI à cible exacte (fixes + favorite, la plus contrainte en premier, candidats au coût marginal le plus bas) — **et, dans la même passe, le quota de base (1, garantie de variété) de toute OI dont les candidats sont concentrés dans ≤ 40 % des slots** (`EX_SCARCE_AVAIL_RATIO`) : sans ça, la phase 2 (dominée par la richesse documentaire slot par slot) attribue presque toujours cette OI au même slot d'une génération à l'autre — pas forcément celui qui en offre le plus, mais celui où son coût la rend compétitive face aux autres candidats de CE slot précis, ce qui n'a rien à voir avec une répartition équitable entre ses slots éligibles (bogue vécu sur P5 : « Situer dans l'espace » n'a de candidats que dans 4 aspects sur 14, et un seul des quatre la « gagnait » 197 fois sur 200 générations avant ce correctif) — **et, dans la même passe, le quota de base de toute OI ayant un sous-tag à ≤1 pt en face d'un sous-tag à ≥2 pts** (`exSoustagCostSkew`) : sans ça, le sous-tag le moins cher gagne quasi systématiquement quand l'OI n'obtient qu'une occurrence, le plus cher n'apparaissant qu'en 2ᵉ occurrence « bonus » (vécu sur P5 : « Situer dans le temps »/« Situer dans l'espace », 0 % de sous-tag cher en occurrence unique sur 500 générations). Le choix du sous-tag pour ces OI (`fairnessOis`, pas les cibles fixes/favorite préexistantes) est tiré au sort pondéré par l'inverse du coût (2× plus cher = 2× moins de chances) — un tirage à poids égal rendait le budget infaisable la plupart du temps quand plusieurs OI sont promues à la fois. **Ce tirage a besoin de marge budgétaire pour survivre au mécanisme de réessai** : à budget pile (ex. 25 pts), le premier essai qui réussit est presque toujours un tirage bon marché (il réussit bien plus souvent), écrasant la pondération malgré elle — vérifié 0 % de sous-tag cher effectivement présent à 25 pts sur P5, contre ~77 % à 30 pts (voir `EX_MAX_POINTS_BY_PERIODE` dans examen.html) ; (2) glouton MRV habituel pour le reste (slots aux candidats les plus rares traités en premier) ; (3) maximise le budget utilisé si de la marge reste — d'abord remplace un candidat par une alternative plus chère de la même OI sur le même slot, puis (3b) échange lequel de deux slots assignés à des OI différentes héberge quelle OI (utile quand le slot d'une OI n'offre qu'un seul sous-tag donc aucune alternative plus chère SUR CET ASPECT, alors qu'un autre slot en offre une) — jamais de changement des comptes par OI, ni de dépassement de `maxPoints`. `relaxDiversityOis` (`Set`, optionnel) désactive la règle « jamais deux fois le même sous-tag » pour les OI listées — exception scopée à un scénario précis (voir `EX_FAVORI_SCENARIOS_BY_PERIODE`), jamais une désactivation globale. Retourne `null` si un slot n'a plus de candidat valide (déjà utilisé, clé de diversité déjà prise pour cette OI — sauf relâchée, plafond d'OI atteint, budget de points dépassé), ou si l'OI favorite/les OI à cible fixe ne totalisent pas exactement leur cible en fin de construction |
...
| `exGenererExamen({questions, periode, aspects, oiList, favoriOi, maxPoints, rng})` | (1) Essaie d'abord les scénarios propres à (période, favori) — voir `EX_FAVORI_SCENARIOS_BY_PERIODE` — chacun avec ses propres slots (`aspectRepeat` de base + `extraSlots` du scénario) et cibles fixes (base + `targets` du scénario), du plus ambitieux au repli explicite sur la base, favori/cible désactivés pour la construction elle-même (déjà encodé dans les cibles du scénario). (2) Sinon, repli générique : construit les `slots` (`exBuildAspectSlots`, avec `EX_ASPECT_REPEAT_BY_PERIODE[periode]` s'il existe) et l'OI réellement atteignables dans cette période (exclut une OI totalement absente, ex. « Déterminer des changements et des continuités » en P1, et toute OI listée dans `EX_OI_VARIETY_EXCLUDE_BY_PERIODE[periode]`), puis boucle imbriquée `exTryBuild` sur les paliers de cible décroissants pour l'OI favorite (`exFavoriTargetLevels`, boucle extérieure) puis pour les cibles fixes de base (`exFixedTargetLevels`, boucle intérieure) — si le nombre exact visé est infaisable dans le budget de points, redescend d'un cran plutôt que d'échouer. Pour chaque palier, `tryLevel` (interne) ne s'arrête plus au premier essai valide mais garde le MEILLEUR des `EX_ATTEMPTS_PER_LEVEL` essais (retour immédiat si un essai atteint déjà `maxPoints` pile) — sans ça, l'écart au budget dépendait entièrement de la phase 3/3b de `exTryBuild` (échanges locaux seulement), qui n'y arrivait pas toujours (P5/P6 à budget 30 : seulement 7-46 % des essais pile selon l'OI favorite). Un plafonnement après `STALE_CUTOFF` (400) essais consécutifs sans amélioration borne le coût de cette recherche élargie pour les scénarios structurellement sous le budget (ex. « Déterminer des changements » en P1, dégrade toujours à 22/25) — sans lui, ces scénarios auraient dépensé le plafond dur `EX_ATTEMPTS_PER_LEVEL` en entier à chaque palier de dégradation testé. Trie le résultat selon l'ordre canonique des aspects du programme |
| `EX_FAVORI_SCENARIOS_BY_PERIODE` | Scénarios propres à une période ET à l'OI favorite (la clé `null` couvre « aucune préférence »), essayés en premier par `exGenererExamen` (voir ci-dessus) — chaque niveau ajuste `targets` (cibles fixes), `extraSlots` (occurrences en plus d'un aspect déjà répétable), `relaxDiversity` (désactive « jamais deux fois le même sous-tag » pour les OI listées, EXCEPTION explicite demandée par l'enseignant — jamais un relâchement global) et/ou `aspectPin` (épingle une OI précise sur un aspect précis, voir `exBuildAspectSlots`). P1 n'a plus d'entrée ici depuis le 2026-08-19 (voir P1 plus bas). P3 : pas de cible fixe de base — chaque favori ajoute +1 occurrence (Causes/Différences/Causalité, cible à 2 seulement, largement sous la limite de sous-tags disponibles donc pas de `relaxDiversity` requis) + 1 Établir des faits (sauf causalité), sur 2 slots supplémentaires toujours portés par « Acte de Québec » et « Situation sociodémographique » (2 des 4 aspects à plus grand nombre de candidats). Tous atteignent 25 points exactement (`exTryBuild` phase 3/3b s'en charge, y compris via l'échange de slot 3b quand le sous-tag reste contraint sur l'aspect assigné). `null` (aucune préférence, ajouté le 2026-08-19) : sans scénario dédié, P3 n'avait que ses 12 créneaux de base (1 par aspect) et plafonnait structurellement à 24 points (0/60 essais à 25, y compris à budget illimité) — les 2 mêmes créneaux supplémentaires que les autres favoris, sans aucune cible fixe (juste plus de marge pour la maximisation de budget), suffisent : 25 points atteint pile (100/100) |

**P1 — Situer dans le temps / dans l'espace réintégrées (2026-08-19)** : P1 avait initialement `EX_OI_VARIETY_EXCLUDE_BY_PERIODE`/`EX_OI_FIXED_TARGET_BY_PERIODE`/`EX_FAVORI_SCENARIOS_BY_PERIODE` dédiés qui laissaient volontairement Situer dans le temps/l'espace de côté (distribution forcée Causes/Causalité/MRF/Différences à 2, Faits à 3, sur 11-13 créneaux déjà saturés). Retiré : ces 3 overrides sont maintenant vides pour P1. Raison : « Déterminer des changements et des continuités » n'a AUCUN candidat en P1 (0 question dans les données) — le maximum réellement atteignable est donc déjà 7 OI sur 8, exactement le nombre d'aspects du programme (7). La variété générale standard (1 par OI atteignable) suffit seule à couvrir les 7 OI ET les 7 aspects, sans aucune configuration spéciale — `EX_ASPECT_REPEAT_BY_PERIODE['P1']` (inchangé : Rapports sociaux ×3, Premiers occupants ×2, Premiers contacts ×2 = 11 créneaux) reste la seule config P1-spécifique nécessaire. Validé à 200/200 (aucune préférence + les 3 favoris avec cible fixe) sur 25 points. Contrepartie acceptée : l'OI favorite (Causes/Différences/Causalité) plafonne à 2 occurrences plutôt que sa cible ambitieuse de 3 — les 11 créneaux sont saturés dès qu'on garantit les 7 OI, donc une question de plus pour la favorite entrerait directement en compétition avec Situer dans le temps/l'espace pour le même créneau ; alternative jamais retenue (essayée puis abandonnée) : ajouter des créneaux/cibles fixes dédiés pour retrouver la cible de 3, mais chaque configuration testée réduisant/réorganisant les cibles pour faire de la place orphelinait « Prise de décision chez les Autochtones » (dépend exclusivement de Mettre en relation des faits/Établir des faits, tous deux alors plafonnés trop bas) — non poursuivi vu le compromis favorable (7/7 OI et aspects garantis à 100 %, contre une cible de favori réduite de 3 à 2 seulement 3 favoris sur 8). Budget : le plafond naturel de la config à 11 créneaux était très exactement 22 points (testé 20-30, toujours 22 pile dès que le budget l'autorise). Porté à **25 points** (chiffre rond demandé par l'enseignant, 2026-08-19) en ajoutant un 12e créneau sur « Alliances et rivalités au sein des Premières Nations » (5 OI distincts, le plus riche restant à 1 seul créneau — voir `EX_ASPECT_REPEAT_BY_PERIODE`) — validé 100/100 pile pour aucune préférence, Causes et Différences comme favori. EXCEPTION acceptée : Causalité comme favori plafonne à 24/25 (25 mathématiquement hors de portée pour ce favori précis avec cette config, vérifié par recherche exhaustive à budget illimité — tous ses candidats coûtent 3 points).
